        ...
        <div class="row">
            <div class="dashboard-info-box">
                ...
                <div class="dashboard-info color-1">
                    <h4>{{$AddList->where('user_id',$userId)->where('status',1)->count()}}</h4> <span>Anúncios  Ativos</span>
                </div>
                <div class="dashboard-info color-2">
                    <h4>{{$AddList->where('user_id',$userId)->where('status',0)->count()}}</h4> <span>Anúncios Pendentes</span>
                </div>
                <div class="dashboard-info color-3">
                    <h4>{{$AddList->where('user_id',$userId)->where('status',2)->count()}}</h4> <span>Anúncios Vencidos</span>
                </div>
                
                <div class="dashboard-info color-5">
                    <h4>{{$AddList->where('user_id',$userId)->sum('reviews')}}</h4> <span>Número de visualizações</span>
                </div>
